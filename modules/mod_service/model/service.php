...
class ServiceModel {
    ...
    public function getServiceList() {   
       ...
       if (isset($_GET['searchForm']['user_name']) && trim($_GET['searchForm']['user_name']) != "") {
            ...
            $where .= " (u.product_model like %ss_product_model or u.product_name like %ss_product_name) ";
...
//        pre($_GET);exit;
      ...
      //  $where .= " and  u.roll_id = 7 ";
       //DB::debugMode();
        ...
        UTIL::doPaging("totalUsers", "u.id,u.product_model,u.product_name,u.region_id,u.warranty_start_date,u.warranty_end_date,u.sell_date,  sr.name as region_name ", CFG::$tblPrefix . "service as u left join ".CFG::$tblPrefix."region as sr ON sr.id=u.region_id", $where, $whereParam, $orderBy);
    ...
    public function getServiceData($id) {
        ...
        $result=DB::queryFirstRow("SELECT product_model,product_name,region_id,DATE_FORMAT(warranty_start_date, '%d-%m-%Y') as warranty_start_date,DATE_FORMAT(warranty_end_date, '%d-%m-%Y') as warranty_end_date,DATE_FORMAT(sell_date, '%d-%m-%Y') as sell_date,qty  FROM " . CFG::$tblPrefix . "service where id=##d ", $id);
        DB::$param_char = '%';
        return $result;
...
//                "qty" => $_POST['qty']
           ...
           //insert new record
            ...
            {   
                ...
                $arrFields['created_date'] = date("Y-m-d H:i:s");
                 ...
                 DB::insert(CFG::$tblPrefix."service",Stringd::processString($arrFields));

                 //get current id
                 APP::$curId=DB::insertId();
                 // pass action result
                    $_SESSION["actionResult"] = array("success" => "Service has been saved successfully");

                   if ($_POST['edit'] == 1)
                   {
                       UTIL::redirect(URI::getURL(APP::$moduleName, "service_edit"."&userid=".$_REQUEST['userid']."&id=".APP::$curId));
                   }else{
                       UTIL::redirect(URI::getURL(APP::$moduleName, "service_list")."&userid=".$_REQUEST['userid']); 
                   }
            }
            else
            {
                ...
                // Update Record
                DB::update(CFG::$tblPrefix."service",Stringd::processString($arrFields)," id=%d ",APP::$curId);
               
            }
             $_SESSION["actionResult"] = array("success" => "Service has been saved successfully");

            if ($_POST['edit'] == 1)
                UTIL::redirect(URI::getURL(APP::$moduleName, "service_edit", APP::$curId));
            else
                UTIL::redirect(URI::getURL(APP::$moduleName, "service_list"));
    ...
    function deleteService() {
        ...
        foreach ($_POST['status'] as $key => $val) {
            ...
            DB::query("delete from " . CFG::$tblPrefix . "service where id=%d", $key);
        }
   ...
   function getValue($table,$field,$where,$wherevalue)
   {
      ...
      function getUserRegion(){
        return DB::query("SELECT id,name FROM " . CFG::$tblPrefix ."region WHERE find_in_set(id,(select group_concat(region_id) from " . CFG::$tblPrefix ."service))");
    }
