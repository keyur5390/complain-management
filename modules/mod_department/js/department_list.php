...
//    $("#parent_department").empty();
//    $("#parent_department").append('<option value="0">Root</option>');
//    var selectedParentdepartment="";
//    $.each(pageData['parentdepartmentData'], function(parentdepartmentKey,parentdepartmentValue) {
////        alert(parentdepartmentValue.name);
//        $("#parent_department").append('<option value="'+parentdepartmentValue.id+'" ' + selectedParentdepartment + ' >'+parentdepartmentValue.name+'</option>');
//    });
            ...
            for (i = 0; i < data.records.length; i++)
            {
                ...
                list = list + '<div class="pageName">';
                 <?php if(Core::hasAccess(APP::$moduleName, "helpDesk_edit")) { ?>
                        list = list + '<a href="index.php?m=mod_department&a=department_edit&id=' + data.records[i].id + '"  title="' + data.records[i].name + '">' + data.records[i].name + '</a>';
                <?php } else { ?>
                        list = list + data.records[i].name;
                <?php } ?>
                list = list + '<ul class="optUl">';
                <?php if(Core::hasAccess(APP::$moduleName, "department_edit")) { ?>
                list = list + '<li><a href="index.php?m=mod_department&a=department_edit&id=' + data.records[i].id + '" title="Edit">Edit</a></li>';
                <?php } ?>

                <?php if(Core::hasAccess(APP::$moduleName, "change_status")) { ?>
                list = list + '<li><a href="#" onclick="return changeStatus(\'index.php?m=mod_department&a=change_status\',' + data.records[i].id + ',\'\')" title="Change Status">Change Status</a></li>';
                <?php } ?>

                <?php //if(Core::hasAccess(APP::$moduleName, "delete_department")) { ?>
                list = list + '<li class=""><a href="#" onclick="deleteRecord(\'index.php?m=mod_department&a=delete_department\',' + data.records[i].id + ')" title="Delete">Delete</a></li>';
                <?php //} ?>    
                list=list+'</ul>';
                list = list + '<a class="toggleLink" href="javascript:;"></a>';
                 list = list + '</div>';
               /* list = list + '<div class="sortOrder" data-title="Sort Order:"><input class="sortTxt text-center inner-page-checkbox" type="text" maxlength="3" id="sort_order'+data.records[i].sort_order+'" name="sort_order['+data.records[i].id+']" value="'+data.records[i].sort_order+'" onblur="if(this.value!='+data.records[i].sort_order+'){changeSortOrder(\'index.php?m=mod_department&a=save_sortorder\',this.value,'+data.records[i].id+');}if(this.value.length==0){this.value='+data.records[i].sort_order+';}" onkeypress=\'return isNumberic(event);\' /></div>';*/
...
//                {
//                    alert("Please enter department name for search");
//                    return false;
//                } 
//                else 
//                {
...
//                {							
//                   alert("Please enter department name or select hepl desk or parent department for search");
//                   return false;
//                }
//            }
            if($("#name").val() == "")
        ...
        function restoreSearchForm()
        {
            if (oldPageState && oldPageState.searchData)
            {
                ...
                $("#parentdepartment").val(oldPageState.searchData.searchForm.parentdepartment);
