...
//                    list = list + '<li><a href="#" onclick="return changeStatus(\'index.php?m=mod_equipment&a=change_status\',' + data.records[i].id + ',\'\')" title="Change Status">Change Status</a></li>';



                    list = list + '<li class="delLi "><a href="#" onclick="deleteRecord(\'index.php?m=mod_equipment&a=delete_equipment\',' + data.records[i].id + ')" title="Delete">Delete</a></li>';
...
//                list = list + '<div class="statusBox" data-title="Status:"><span class="conSpan">' + displayStatus(data.records[i].status) + '</span></div>';
        ...
        $("#btnsub").click(function() {
            if ($.trim($("#txtequipment").val()) == "")
                ...
                var name = $('#txtequipment').val();
                var pid = $('#txtid').val();
                $.ajax({
                    type: "POST",
                    url: '<?php echo URI::getURL("mod_equipment", "save_equipment"); ?>',
                    data: {pname: name, pid: pid},
                    catch : false,
                    success: function(data)
                    {
                        $('#btnsub').val('Add Type of equipment');
                        $('#txtequipment').val('');
                        $('#txtid').val('');
...
//     @author Sagar Jogi <sagarjogi.datatech@gmail.com>
 ...
 $("#btncnl").click(function() {
    ...
    $('#txtid').val('');
        ...
        // display message			
<?php Core::displayMessage("actionResult", "equipment"); ?>
...
//     @author Sagar Jogi <sagarjogi.datatech@gmail.com>
    ...
    function update_type(temp) {
        ...
        $('#txtequipment').val(name);
        $('#txtid').val(id);
        $('#btnsub').val('Update Type of Equipment');
