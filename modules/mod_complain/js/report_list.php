        ...
        var displayPages = function(data, startIndex) {
            ...
            for (i = 0; i < data.records.length; i++)
            {
               ...
               /* list = list + '<div class="sortOrder" data-title="Sort Order:"><input class="sortTxt text-center inner-page-checkbox" type="text" maxlength="3" id="sort_order'+data.records[i].sort_order+'" name="sort_order['+data.records[i].id+']" value="'+data.records[i].sort_order+'" onblur="if(this.value!='+data.records[i].sort_order+'){changeSortOrder(\'index.php?m=mod_complain&a=save_sortorder\',this.value,'+data.records[i].id+');}if(this.value.length==0){this.value='+data.records[i].sort_order+';}" onkeypress=\'return isNumberic(event);\' /></div>';*/
                ...
                list = list + '<div class="">' + displayDate(data.records[i].recived_time) + '</div></li>';
                   list = list + '<li ' + odd + '><div></div><div></div>';
                   //alert(data.records[i].reach_time);
                list = list + '<div class="hashBox" style="width:200px;"><span class="topLi " style="padding-top: 8px; PADDING-BOTTOM: 8px;margin-bottom: 8px;  float: left;  text-align: center;width: 140px; ">Reach Time</span>'+(data.records[i].reach_time) +'</div>';
                list = list + '<div class="hashBox" style="width:200px;"><span class="topLi " style="padding-top: 8px; PADDING-BOTTOM: 8px;margin-bottom: 8px;  float: left;  text-align: center;width: 140px; ">Close Time</span>'+(data.records[i].close_time) +'</div>';
                var opnCls = (data.records[i].status  == 0) ? "Open" : "Close";
               // var cls = (data.records[i].status  == 1) ? "selected='selected'" : " ";
                list = list + '<div class="hashBox" style="width:200px;"><span class="topLi " style="padding-top: 8px; PADDING-BOTTOM: 8px;margin-bottom: 8px;  float: left;  text-align: center;width: 100px; ">Status</span><br/>'+opnCls+'</div>';
                ...
                list = list + '<div class="hashBox" style="width:200px;"><span class="topLi " style="padding-top: 8px; PADDING-BOTTOM: 8px;margin-bottom: 8px;  float: left;  text-align: center;width: 100px; ">Remark</span><br/>'+data.records[i].remark+'</div>';
                list = list + '<div class="hashBox" style="width:200px;"><span class="topLi " style="padding-top: 8px; PADDING-BOTTOM: 8px;margin-bottom: 8px;  float: left;  text-align: center;width: 100px; ">logged By</span><br/>'+data.records[i].updatedBy+'</div>';
                var clsby = (data.records[i].close_by!=null)?data.records[i].close_by:"N/A";
                list = list + '<div class="hashBox" style="width:200px;"><span class="topLi " style="padding-top: 8px; PADDING-BOTTOM: 8px;margin-bottom: 8px;  float: left;  text-align: center;width: 100px; ">Close By</span><br/>'+clsby+'</div>';
                //list = list + '<div class="hashBox" style="width:200px;"><a href="#" id="btnForward" class="trans comBtn" onclick="forwardCom('+data.records[i].comid+')" title="Forward">Forward</a><br/></div>';
...
//            return true;
    ...
    function exportlist()
    {
        window.location.href="<?php echo CFG::$livePath."/page.php"; ?>?action=exportlist&name="+$('#name').val()+"&date_from="+$('#date_from').val()+"&date_to="+$('#date_to').val()+"&complain_type="+$('#complain_type').val()+"&station_name="+$('#station_name').val()+"&department_name="+$('#department_name').val()+"";
//        $.ajax({
//                url: "<?php echo CFG::$livePath."/page.php"; ?>",
//                type: "post",
//                data:"action=exportlist&name="+$('#name').val()+"&date_from="+$('#date_from').val()+"&date_to="+$('#date_to').val()+"",              
//                "success": function(data) {
//                    if(data!='')
//                    {
//                        
