...
//        die();
    ...
    */
    ...
    {
        if((int)$id != 0){
        return DB::queryFirstRow("SELECT r.id,r.name,r.code,r.sort_order,r.status,r.description,r.type FROM ".CFG::$tblPrefix."station as r where r.id=%d ",$id);}else{
            return false;
        }
    }
...
//        pre($_POST,1);
        if(isset($_POST['name'])!='')
        {
            ...
            $arrFields=array("name"=>$varName,"code"=>str_replace(" ","-",$varCode),"description"=>$varDescription,"type"=>$vartype,"status"=>$varStatus,"sort_order"=>$sortOrder,"updated_by"=>$_SESSION['user_login']['id'],"updated_date"=>date("Y-m-d H:i:s"));
