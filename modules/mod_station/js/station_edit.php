...
<?php echo Stringd::createJsObject("pageData", $data); ?>
    ...
    $(document).ready(function() {
        ...
        var user_id = '';
        if (pageData['stationData'])    // fill form data if passed
        {
            user_id = pageData['stationData'].id;
        }
...
//                engineer: {required: true}
            ...
            var messages = {
                name: "Please enter station name",
                code: 
                {
                    required: "Please enter station code",
                    minlength: "station code must have at least 3 characters",
//                    usernameValue: "Username should not start with underscore (_)",
                    whitespaceValue: "White space not allowed in station code",
                    remote: "station code already exist with this station code"
                },
//                  engineer:"Please select engineer"
            };

            // initialize form validator
            //validaeForm("frmstation", rules, messages);
...
<?php Core::displayMessage("actionResult", "Station Save"); ?>
            ...
            $(".progControlSelect2").select2({
                placeholder: "Enter Parent Station"//Placeholder
            });
    ...
    function fillForm()
    {
        if (pageData['stationData'])    // fill form data if passed
        {  
           console.log(pageData['stationData']);
            $("#name").val(pageData['stationData'].name);
            $("#code").val(pageData['stationData'].code);
            //var equipmentdb = pageData['stationData'].equipment.split(',');
            
//            $("#equipment").val(pageData['stationData'].equipment);
            $("#dealer").val((pageData['stationData'].dealer!=null)?pageData['stationData'].dealer:"0");
//            $("#description").val(pageData['stationData'].description);
//            $("#parent_station").val(pageData['stationData'].parent_station);
//            $("#helpDesk").val(pageData['stationData'].helpDesk);
//            $("#engineer").val(pageData['stationData'].engineer);
            
            
            
            //$("#sortOrder").val(pageData['stationData'].sort_order);
            
//            txtHelpDesk
//            txtEngineer
            
            if (pageData['stationData'].status == 1)
            {
                $("#chkStatus").attr("checked", "checked");
                $("#checkAct").html("Active");
            }
            else if (pageData['stationData'].status == 0)
            {
    ...
    function SelectedFunctionality(Parentstation)
    {   
        if($("#parent_station").val()=="0")
        {
            $('.helpDesk').show();
            $('.engineer').hide();
            $('#engineer').val('');
...
//                        alert(data);
        ...
        */
        ...
        $('#parent_station').on('change', function() {
      
        var val=this.value;
        if(val!='')
        {
            $.ajax({
                url: "<?php echo URI::getURL(APP::$moduleName, 'station_edit'); ?>",
                type: "post",
                data:"station_id="+ val+"&ajaxEngineer=yes",              
                "success": function(data) {
                    ...
                    if(data!='')
                    {
                        ...
                        $("#engineer").empty();
                        $("#engineer").append('<option value="">Please Select station</option>');
...
<?php // include CFG::$absPath . '/' . CFG::$helper . "/templates/js/add_new_parent_station.php"; ?>