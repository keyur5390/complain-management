...
//        }
    ...
    */
    function getstationList()
    {
        ...
        $wherestation = "";
        if (isset($_GET['searchForm']['parentstation']) && trim($_GET['searchForm']['parentstation']) != "") 
        {
            if(empty($where))
            {
                if($_GET['searchForm']['parentstation']!="") 
                    $wherestation.=" r.parent_station = ".$_GET['searchForm']['parentstation'] . " ";
            }
            else
            {
                if($_GET['searchForm']['parentstation']!="")
                $wherestation.=" and r.parent_station = ".$_GET['searchForm']['parentstation'] . " ";
            }
        }
        $where.=$wherestation;
...
//        die();
    ...
    */
    ...
    {
        if((int)$id != 0){
        return DB::queryFirstRow("SELECT r.id,r.name,r.code,r.sort_order,r.status,r.dealer ,r.equipment  FROM ".CFG::$tblPrefix."station as r where r.id=%d ",$id);}else{
            return false;
        }
    }
...
//        pre($_POST,1);
        if(isset($_POST['name'])!='')
        {
            ...
            $arrFields=array("name"=>$varName,"code"=>str_replace(" ","-",$varCode),"dealer"=>$vardealer,"equipment"=>$varequipmentDa,"status"=>$varStatus,"sort_order"=>$sortOrder,"updated_by"=>$_SESSION['user_login']['id'],"updated_date"=>date("Y-m-d H:i:s"));
